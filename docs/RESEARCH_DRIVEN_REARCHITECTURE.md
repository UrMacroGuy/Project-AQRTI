...
`backend/strategies/strategy_generator.py::_GENERATORS` — 6 named templates replaced the old 8 random-mutation families: `post_earnings_drift` (PEAD), `momentum_trend` (Jegadeesh & Titman / Moskowitz-Ooi-Pedersen), `mean_reversion_quality` (Connors RSI(2), substituted with adapted rsi_14), `event_catalyst` (event-study drift), `regime_dca_timing` (allocation-tilt signal, not a standalone trade), `rotation_monitor` (cross-sectional RS, uses `relative_strength_nifty_21d` as a documented proxy since the DSL can't do true cross-symbol comparison).

**Walk-forward validation (12-fold, 504d/63d, `backend/scripts/walk_forward_templates.py`): 0 of 6 templates currently pass WFO Sharpe ≥ 0.7.** 5 produced zero trades (blocked by fail-closed DSL on missing research/regime history — now accumulating daily since the `.env` fix above). `rotation_monitor` traded and failed (Sharpe -4.6/-5.6). **Correct, honest state — no gate weakened.** Re-run `walk_forward_templates.py` periodically; research history should start showing real coverage within days/weeks now that synthesis runs automatically every boot and every trading day.

`MIN_OOS_WIN_RATE = 50.0` (`promotion_config.py`) stacks on every existing gate at `promote_strategy()`. Unit-tested, 52 tests total in `test_core.py`, all passing.
