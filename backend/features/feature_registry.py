...
FEATURE_CATALOG: List[FeatureDef] = [
    ...
    FeatureDef("return_126d",             "price", "126-day (6-month) simple return (%) — 52wk-high momentum confirmation horizon",
               "((close_t / close_t-126) - 1) * 100", ["close"], 127),
    FeatureDef("tom_window",              "price", "Turn-of-month flag: 1 if among first 3 trading days of the month, else 0",
               "1 if trading_days_elapsed_this_month <= 3 else 0", ["date"], 1),
    FeatureDef("trend_tstat_63d",         "price", "t-statistic of 63-day drift: mean(ret)/std(ret)*sqrt(63) — statistical trend significance",
               "mean(daily_return, 63) / std(daily_return, 63) * sqrt(63)", ["close"], 64),
