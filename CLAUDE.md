...
- **Known open tension:** the 50% WR floor structurally excludes low-WR/high-payoff momentum families (lab measured +3.6%/trade at 43.8% WR on 52wk-high momentum — the strongest raw edge found). Changing the floor to an expectancy-based variant is a USER decision; flag it, never make it unilaterally.
- Current honest baseline: **0 of 7 templates pass WFO** — mostly zero-trade (research/regime feature history still accumulating), one traded and failed. That is correct behavior, not a bug. Re-run `walk_forward_templates.py` monthly as `research_synthesizer.synthesize_all()` accumulates history.
- A promoted algo keeps its signal stream only while: rolling-20-trade WR ≥ 50%, live drawdown ≤ 1.5× validated backtest max_drawdown, and the current regime is one it was validated in (`backend/strategies/live_validator.py`).
