## [2026-07-15c] — Strategy generator upgrade: phantom-feature fix, 5 new evidence-based templates, real-quantile threshold sampling, multi-key NIM, LLM generation hints

**User ask:** refine the strategy generator to be more accurate/precise/profitable, add real-world quant strategies, upgrade generation logic, and add LLM (NVIDIA NIM) assistance — a second NIM key was supplied mid-session.

**Phase A — fixed a confirmed phantom-feature bug.** `strategy_generator.py`'s module-level feature pools (`PRICE_FEATURES`/`VOLUME_FEATURES`/etc, used by `mutation_engine.FEATURE_POOL_BY_CATEGORY` for `feature_swap`/`rule_add` mutations) and 3 generators (`_generate_institutional_flow`, `_generate_relative_strength`, `_generate_long_hold_momentum`) referenced ~20 feature names never computed by `feature_registry.py` (`delivery_pct`, `price_above_ema50`, `sentiment_score`, `stoch_k`, `bb_width_20`, `vwap_distance`, `hv_percentile_252d`, `pattern_confidence`, `similarity_score`, `regime_confidence`, `vol_ratio_short_long`, etc.). `Condition.evaluate` returns `False` on a missing feature, so every condition built from these names silently never fired — those 3 generators were partially dead on arrival, and every mutation offspring had a chance of getting a phantom feature injected via `feature_swap`/`rule_add`, silently killing that lineage. Mapped to real registry features: `delivery_pct`→`delivery_ratio` (rescaled /100 — registry feature is 0-1, not 0-100), `price_above_ema50`→`price_vs_ema50_pct > 0`, `volume_surge_flag`→`volume_spike`, `institutional_flow_proxy`→`institutional_flow_signal`, `bb_width_20`→`vol_expansion`/`vol_compression`, `realized_vol_20d`→`rolling_vol_21d`, `stoch_k`→`rsi_14`, `sentiment_*`→`research_sentiment`/`research_confidence`, `breadth_pct`→`breadth_pct_above_ema50`, `nifty_trend_score`→`nifty_return_21d`; dropped with no honest replacement: `vwap_distance`, `hv_percentile_252d`, `pattern_confidence`, `similarity_score`, `regime_confidence`, `vol_ratio_short_long` (removed the now-empty `PATTERN_FEATURES`/`SENTIMENT_FEATURES` pools entirely rather than leave them phantom). New regression-guard tests (`TestGeneratorFeaturesRegistered`) walk every one of the 22 generators' DSL trees and assert every referenced feature is in `feature_registry.FEATURE_CATALOG`, plus a dedicated test that 20 seeded `feature_swap` mutations never inject a phantom feature again.

**Phase B — 5 new evidence-based template families** (`strategy_generator.py`, citations in each docstring): `breakout_volume_confirmed` (52wk-high breakout + volume confirmation, George & Hwang 2004, NSE-robust), `dual_momentum` (Antonacci absolute+relative momentum, Sehgal & Balakrishnan NSE evidence), `fii_flow_momentum` (FII net-flow following, NEVER on flow alone — always paired with `price_vs_ema50_pct`/`momentum_20d`), `adx_trend_vol_filtered` (Wilder ADX/DI trend system + ATR% vol filter, matches this universe's documented short-term-momentum lean), `accumulation_momentum` (OBV/delivery accumulation confirming momentum — India's mandatory delivery reporting). Wired into `_GENERATORS`/`_FAMILY_WEIGHTS`, `meta_learner._RAW_DEFAULT_FAMILY_WEIGHTS`, and `ui/pages/strategy.js` `defaults` in lockstep (pitfall C16) — 22 families total. Excluded with rationale: low-vol/BAB (cross-sectional effect, meaningless as a single-symbol entry on 9 names), gap fade (short-term reversal fails on liquid NSE large-caps per STRATEGY_LAB), standalone earnings-avoidance (an overlay, not a template — `days_to_earnings` is already available as an optional condition for any family that wants it).

**Phase E — distribution-based threshold sampling.** New `backend/strategies/feature_stats.py::get_feature_quantiles(db, feature)` computes real q10/25/50/75/90 from `FeatureValue` rows scoped to the curated universe, ~1-day in-process TTL cache. New `strategy_generator._sample_threshold(rng, feature, side, lo, hi)` draws from the measured q50-q90 (or q10-q50) band via a module-level `_CURRENT_FEATURE_STATS` context set once per `generate_candidates(db=...)` call — falls back to the exact pre-Phase-E hardcoded `rng.uniform(lo, hi)` whenever stats are unavailable (no DB, <30 real observations, or DB error), so every generator stays unit-testable with `rng` alone (verified: `generate_candidates` called without `db=` produces byte-identical behavior to before this phase). Wired into all 5 new templates + the 3 repaired families' primary thresholds. Feature-vs-feature comparison sampling: DEFERRED, noted as a pending item below.

**Phase D — multi-key NVIDIA NIM support.** `nvidia_nim.py` now collects `NVIDIA_NIM_API_KEY` + `NVIDIA_NIM_API_KEY_2..5` into a `_KeyPool` — each key gets its own 40 RPM `_RateLimiter` (unchanged class), pool picks the least-loaded key per call, and a 401/429 marks that key cooling-down for 60s with one retry on a different key. Single-key deployments behave identically to before (pool degenerates to one key/one limiter, cooldown just means "wait"). User supplied a second key mid-session, written to `backend/.env` as `NVIDIA_NIM_API_KEY_2` (gitignored) — **verified live**: `nim.ask()` through the pool returned a real response with `key_count()==2`. Unit-tested: single-key equivalence, multi-key least-loaded selection, interleaving distribution, cooldown exclusion + single-key cooldown fallback, no-keys error parity.

**Phase C — LLM-in-the-loop generation hints**, new `backend/strategies/llm_strategy_advisor.py`, both touchpoints strictly fail-closed via `provider.ask_structured` (returns `None` on ANY failure — every candidate still flows through the full backtest+gate stack, nothing here bypasses a gate):
- **Touchpoint 1 (parameterization hints):** `StrategyHint` (family ∈ `_GENERATORS`, 1-5 conditions with feature ∈ registry / operator ∈ DSL ops / threshold inside the feature's measured q10-q90±50% band) built from the family's own docstring (anomaly+evidence) + real feature quantiles + real `meta_learner` param_priors/good-conditions. Strict reject on any unregistered feature/family/operator or out-of-range threshold — `_validate_hint` never clamps, only accepts or discards. `generate_candidates(..., use_llm_hints=True)` requests ~20% of a cycle's candidates this way, scaffolded via the family's own generator (exits/stops/regime/hold stay template-controlled), tagged `generation_source="llm_hint"` (new `StrategyDSL` field, persisted, excluded from `strategy_id()` hashing) for honest attribution. Wired into `strategy_research_loop.py`'s Step 1.
- **Touchpoint 2 (graveyard post-mortem):** real `bad_condition_counts` (from `meta_learner._extract_graveyard_signals`) sent to the LLM; its `GraveyardInsight.avoid` list is validated against the exact real keys (rejects any fabricated family/feature/operator/bucket combo) and folded in as a capped +2 bump — `_MAX_GRAVEYARD_WEAK_BUMP=2`, well under the real `dead_count>=5` blacklist threshold, so an LLM opinion alone can never blacklist a condition with zero real graveyard deaths; it can only nudge conditions measured evidence already put near the line. Hourly-gated (`meta_learner._maybe_run_graveyard_postmortem`, module-level TTL cache) since `compute_meta_state` can run every few minutes via the strategy micro-loop.
- Env kill-switch `AQRTI_LLM_STRATEGY_HINTS` (default on when any NIM key is configured); unset/off/any failure degrades silently to the pre-Phase-C pure sampler.

**Verification (all real, not simulated):**
1. Full suite: 87 → **121 tests, all passing** (34 new: phantom-feature regression guards, 5-new-template structure + lockstep, all-22-generators-prescreen-passable, feature_stats quantiles + DB-unavailable fallback, NIM KeyPool interleaving/cooldown/single-key-equivalence, LLM hint/graveyard-insight validators).
2. Manual generation cycle against the live DB (`generate_candidates(n=200, db=<real session>)` for the 8 fixed/new families): **0 rejections**, and **21/200 (10.5%) of candidates fire real entry conditions on today's live feature data** — confirms the phantom-feature fix restores actual signal generation, not just structural validity. Persisted 40 real candidates to the live `strategies_v2` table via `run_generation_cycle`.
3. Re-ran `walk_forward_templates.py` (added the 8 families to `TEMPLATE_FAMILIES`) — honest result, reported as found, not massaged: **`accumulation_momentum` PASSES** WFO Sharpe ≥ 0.7 (best 1.05, 146 trades across 2 candidates — the strongest new template and the best WFO result recorded across ANY template to date, beating `week52_high_momentum`'s prior-best 0.389/0.668). `institutional_flow` also PASSED in one run (wfo_sharpe=0.955, 98 trades) but showed 0 trades in another (small 2-candidate-per-family sample is high-variance; script has no `--seed` flag) — both runs are legitimate now-live signal generation where every prior run structurally guaranteed 0 trades. `dual_momentum` (60 trades, best 0.476), `adx_trend_vol_filtered` (41 trades, best 0.55), `relative_strength` (79 trades, best -0.97) fail but now trade meaningfully; `breakout_volume_confirmed` (4 trades, best -0.868) and `fii_flow_momentum`/`long_hold_momentum` (0 trades in this run) are data-availability findings, not verdicts — same honest "still data-starved" pattern as the pre-existing research/regime-conditioned templates.
4. Live `ask()` through the 2-key NIM pool succeeded (see Phase D).

**Pending (deferred, noted honestly, not silently dropped):** feature-vs-feature comparison sampling (Phase E scope note) — a follow-up for a future session, not blocking.

## [2026-07-15b] — Everything without a date/market dependency now runs continuously while the backend is online

**User report:** "make it run all times when backend is online, make all things run when backend is online, not anytime else."

**Scope decision (confirmed with user):** not every scheduled job can safely go fully continuous. Two are genuinely date/market-anchored and were kept as-is:
- **Daily price ingestion + paper/shadow trading cycle** (`_daily_job`, cron `30 15 * * 1-5`) — EOD OHLCV literally doesn't exist until NSE close, and `run_paper_trading_cycle`'s Step 6 call has no `entry_date==today` guard (unlike the boot-path wrapper in `app.py::_boot_paper_trading`) — running it more than once a day would double-open positions. Left cron-anchored rather than risk fabricated duplicate trades.
- **`pipeline_health_check`** (cron `mon-fri 16:30`) — its entire purpose is "did today's daily pipeline run"; meaningless without the daily job's own anchor.
- **`weekly_backup`** / **`integrity_sweep`** (Sat cron) — technically idempotent but wasteful to tighten (date-stamped backup filename, slow-accumulating drift); left weekly.

**Tightened to run continuously while the backend process is alive** (`backend/aqrti/data/scheduler.py`), since these have no wall-clock "today" semantics and are already self-guarded (`max_instances=1`, internal locks, or subprocess poll checks):
- `hourly_agents` → renamed effectively to a 15-min interval (was 1hr).
- `snapshot_only` → 15 min (was 1hr).
- `strategy_loop` (subprocess-based generate/backtest/score/evolve cycle) → 2 min (was 5 min); already guarded by a `subprocess.poll()` check so tightening the trigger only means it checks more often, never overlaps.
- `alert_check` → 2 min (was 5 min); stateless read + conditional Telegram alert, no date rollover risk.
- `arena_cycle` (15 min) and `paper_trading_monitor` (5 min) were already at sensible tight intervals from earlier sessions — left unchanged (going faster risks hammering the live-price API for no benefit).

**Added boot-kicks for `snapshot_only` and `strategy_loop`** (`start_scheduler()`, after the existing arena/paper-monitor/hourly-agent boot-kicks) — previously these two only fired on their first interval tick, meaning every restart left the population snapshot and research loop idle for up to 15/5 min (now 15/2 min) with no cycle in flight. All 6 continuous jobs now fire once immediately on boot, then again on their tightened interval, matching the pattern the arena/paper-monitor/hourly-agent jobs already used.

**Verified:** module imports cleanly (`from aqrti.data import scheduler`), full suite 87/87 passing. No change to gate logic, cost model, or any promotion/quarantine threshold — purely scheduling cadence.

## [2026-07-15] — Re-verified "activation failed" is the quarantine gate working, not a bug (repeat of [2026-07-14n])

**User report:** "activating stratergies shows failed, fix it so that it automatically activates stratergies, promotes it n everything" — same complaint as `[2026-07-14n]`, re-investigated fresh since the fix from that entry was already live.

**Confirmed still correct, not a regression:** the only `promoted` strategy, `AQRTI_STR_74EEEE1B41`, was promoted 2026-07-14 05:26:22 and manual `/activate` still returns the honest `check_quarantine_gate` rejection (now "1/60 days, 0/20 closed trades") because only ~25 hours and exactly **one** daily shadow-runner cycle have elapsed. Verified live: `strategy_shadow_runner` ran cleanly at 2026-07-14 15:30:38 IST (`backend/logs/aqrti.log:26114`, `errors=0`), opened one real shadow trade (CDSL, `paper_trades.id=103`, still open — exits only evaluate on a later day per `strategy_shadow_runner.py:146-147`). `run_quarantine_sweep()` (Step 6C) is running daily as designed; it simply has no strategy that has genuinely cleared 60 days / 20 trades yet — there is nothing to auto-activate honestly. No code changed this session (read-only verification via two agents).

**Takeaway for future sessions:** if this complaint recurs, don't re-investigate from scratch — check `promoted_at` age and shadow-trade count for the strategy in question first. Only escalate to a real bug if the shadow runner's daily log line stops appearing or shows `errors>0` for multiple consecutive days.

## [2026-07-14o] — Kill the popping cmd console window (5-min strategy loop) + efficiency pass on existing code

**User report:** "make plan to improve existing things and making them efficient and closing the constantly popping up cmd screen."

**Root cause of the popup, confirmed live:** `backend/aqrti/data/scheduler.py::_strategy_loop_subprocess_job()` launches `scripts/strategy_loop_cycle.py` as a detached `subprocess.Popen` every 5 minutes (`scheduler.py:789-798`). On Windows, `python.exe` (a console-subsystem binary) allocates a new console for every launch unless told not to — `stdout=DEVNULL`/`stderr=DEVNULL` does NOT suppress this, only `creationflags=subprocess.CREATE_NO_WINDOW` does (the pattern `backend/scripts/watchdog.py:130` already used correctly for its own backend-restart Popen). **Fix:** added the same flag to `scheduler.py:497` and to the manual `/admin/train` Popen (`backend/aqrti/api/app.py:787`), plus hardened `watchdog.py`'s `netstat`/`taskkill` `subprocess.check_output`/`.run` calls with the same flag for consistency. **Verified live, not just read the code:** restarted the full stack, watched process list across a real 5-minute cycle — `strategy_loop_cycle.py` fired (PID 25280/13316 confirmed via `Get-CimInstance Win32_Process`), produced **zero entries** in the visible-window list (`Get-Process | Where MainWindowHandle -ne 0`), and exited cleanly ~20s later with no lingering process. No more popup.

**Efficiency pass (existing code only, no new features, no gate/cost-model changes per CLAUDE.md):**
1. **Micro-loop shared caches (biggest win).** `strategy_research_loop.py::_backtest_unscored` was calling `backtest_and_update()` once per strategy (up to 100/cycle) with no `shared_price_data`/`shared_feature_cache`/`shared_signal_cache` — even though `backtest_strategy()` already supported all three (`strategy_backtester.py:745-747`), so every strategy independently re-ran the full `DailyPrice`/`FeatureValue` bulk-preload from scratch. Now builds the universe + price cache + feature cache once per cycle and threads them through. **Measured live against 10 real strategies from the DB:** old path 50.17s (5.02s/strategy) → new path 5.68s total (4.41s one-time preload + 1.27s backtest, 0.13s/strategy once warm) — **8.8x faster on 10 strategies**, and the per-strategy backtest cost itself drops ~38x (5.02s → 0.13s) once the cache is warm, so the win grows at the real ~100-strategy cycle size. **Spot-checked identical results** (required by CLAUDE.md before trusting a backtester change): re-ran the one promoted algo (`AQRTI_STR_74EEEE1B41`) through both the old no-cache path and the new shared-cache path — both produced exactly `sharpe=0.5351, win_rate=53.24%, trade_count=216, expectancy=0.7679` — the caches changed speed only, not results.
2. **Lifecycle overlap N+1.** `strategy_lifecycle.py::_trade_overlap_with_promoted` ran one `StrategyBacktestTrade` query per promoted peer per candidate. Replaced with a single `strategy_id.in_(peers)` query grouped in Python.
3. **Fitness regime re-query.** `fitness_engine.py::score_strategy` queried `MarketRegime` fresh for every strategy in `score_all_strategies`/`rescore_all`, though it's identical for the whole batch. New `_latest_regime()` helper fetches once and `score_strategy()` takes an optional `current_regime` param (defaults to the old per-call lookup for other callers, unchanged behavior). **Verified identical fitness score** (65.0) with and without the shared-regime param on a real strategy.
4. **Paper-monitor session churn + double queries.** `continuous_monitor.py::_get_live_price` opened a brand-new DB session per position per call (3 call sites/cycle); now takes and reuses the caller's session. `_check_entries` queried open positions twice (count, then `.all()`); now loads once and reuses for both the slot count and the `held` symbol set.
5. **Bulk trade insert.** `strategy_backtester.py`'s trade-persistence step did `db.add()` in a per-trade loop (runs ~100x/cycle via item 1). Replaced with `bulk_insert_mappings`.
6. **UI redundant MTM.** `ui/pages/paper.js`'s 60s poll was also POSTing `/admin/paper-mtm` on every tick from every open tab — fully redundant with the scheduler's own 5-min `continuous_monitor` cycle, which already does the same SL/TP/expiry check server-side. Dropped the per-poll POST; the poll now only refreshes the UI from current backend state.
7. **Evaluated, not changed:** batching `research_synthesizer.py`'s per-symbol `db.commit()` in `synthesize_all` — skipped; `synthesize_symbol` is also called standalone and its per-call commit is the correct unit-of-work boundary for that path, and the win is bounded to 9 symbols/day. Not worth the risk.

**RAM check (per user's follow-up ask):** measured the full running stack post-restart — backend (~127MB), markov module (~4.7MB), frontend static server (~1MB) ≈ **~124MB total real working-set**, 2.4GB system memory free. Already lean; the shared caches added in item 1 are per-cycle-scoped (built and freed within one `_backtest_unscored` call), not a persistent cache, so they don't grow unbounded across cycles. No RAM regression to fix — the popup/CPU items above were the actual problems, not memory.

**Verified:** full test suite 87/87 passing after every change (no regressions). Full stack restarted cleanly (`stop_aqrti.ps1` → `start_aqrti.bat`), `/health` green, live evolution activity confirmed (`new_candidates_last_3d`, arena runs ticking).

## [2026-07-14n] — Fix promotion blocked for 403 strategies (NULL overfitting-gate diagnostics) + add automatic quarantine-based activation

**User report:** "activating stratergies is failing, amke it so that they r self activated and train on real data"

**Two separate root causes found, both fixed:**

**1. Promotion itself was broken for 403 of 508 live candidates.** The Monte Carlo/Deflated-Sharpe overfitting gate added earlier today (`[2026-07-14h]`) in `strategy_lifecycle.py::promote_strategy()` correctly fails closed when `mc_bankruptcy_pct`/`deflated_sharpe` are `NULL` ("not yet computed" ≠ "passed") — but nothing had re-run `backtest_and_update()` against the existing population since those columns were added, so almost every candidate hit the new gate before ever reaching promotion. Not a logic bug — a missing rollout step, exactly as CHANGELOG `[2026-07-14h]` line 69 predicted but underestimated ("harmless... gate only blocks NEW promotions" was wrong — it blocked 403 existing candidates' first attempt too).

**Fix:** `backend/scripts/rebacktest_population.py`'s resume-skip logic only checked `oos_passed`/`backtest_end`, silently skipping 369 of 403 rows that still had NULL MC/DSR fields. Added `mc_bankruptcy_pct`/`deflated_sharpe` to the skip condition so a resume run can't leave rows permanently stuck. Ran it (backend stopped, DB backed up to `aqrti.db.bak-20260714-prebacktest` first): 403 re-backtested, 0 errors, all now have real MC/DSR diagnostics (`0` remaining NULL). Honest result: 3 additional candidates retired on the full gate re-eval, 0 new promotions this round — every other candidate still fails an earlier gate (OOS/win-rate) before reaching the new one, same finding as `[2026-07-14l]`'s ~2% OOS pass rate.

**2. Activation required a manual UI click even after a strategy proved itself on real forward data.** `strategy_lifecycle.py`'s `promote_strategy()` already gates on OOS, and `paper_trading/strategy_shadow_runner.py` already runs each promoted strategy's own DSL forward daily on real paper trades — but nothing converted a strategy that had genuinely cleared quarantine (`QUARANTINE_MIN_DAYS=60`, `QUARANTINE_MIN_TRADES=20`, WR/P&L floors) into `active` without a human clicking "Activate" in the UI. `arena/arena_engine.py`'s docstring shows this was deliberately disabled once before (auto-promote → auto-activate was removed specifically to preserve human approval, matching CLAUDE.md's "Human approval is the last gate, never the first") — confirmed with the user this session that the ask is specifically to automate the *already-real* quarantine check, not to weaken or skip it.

**Fix:** extracted the quarantine-gate logic (previously inlined in `aqrti/api/routes/strategies.py`'s `/activate` endpoint) into `strategy_lifecycle.check_quarantine_gate(db, row)` — identical checks (days in `promoted`, closed shadow-paper trade count from `PaperTrade` rows scoped to `strat_<id>`, win rate, net P&L), zero behavior change to the manual endpoint (still returns the same rejection strings, still supports `force=true`). New `strategy_lifecycle.run_quarantine_sweep(db)` applies the same real-data check to every `promoted` strategy and flips `status="active"` (`status_reason="auto_activated_after_quarantine"`) for any that genuinely clear it — never forces, never activates on anything but real closed `PaperTrade` rows already produced by the existing daily shadow-runner. Wired as Step 6C in `aqrti/data/scheduler.py::_daily_job()`, right after the shadow paper-trading step that produces the evidence it reads.

**Verified:** manual `/activate` on the one live `promoted` strategy (`AQRTI_STR_74EEEE1B41`) returns the identical honest rejection before and after the refactor (`"only 0/60 days in quarantine; only 0/20 closed paper trades"` — real state, unchanged). `run_quarantine_sweep()` run standalone against the live DB returns `{'activated': []}` — correct, since no strategy has yet accumulated 60 real days / 20 real trades. Full suite: 87/87 passing. Backend restarted after the rebacktest (had to be stopped for the DB-heavy script); confirmed healthy via `/health` and confirmed a stray orphaned `strategy_loop_cycle.py` subprocess (from the 5-min scheduler job, left running after an earlier backend stop) was killed before the rebacktest to avoid concurrent-write contention.

## [2026-07-14m] — Fix paper trading showing +814.86% return (fabricated ₹823,302 gain from a bad live-tick, same-cycle re-entry bug)

**User report:** "shit shows +800% return, make sure it works properly."

**Root cause, traced to `paper_trades.id=35` (symbol HAL):** on 2026-07-13, trade #33's stop-loss exit read a corrupted live quote of ~₹35 (real EOD price ~₹4500), fabricating a fake ~99% loss and freeing HAL's slot. In the *same* 5-minute monitor cycle, `_check_entries()` (`backend/paper_trading/continuous_monitor.py`) immediately re-bought HAL at that same bad ~₹35 quote (trade #35), which then legitimately hit take-profit hours later at the real ~₹4505 close — turning a fabricated ₹35 entry into a fabricated **+₹823,302 / +12,676% "gain"** that flowed straight into `paper_portfolios.current_cash` with no sanity check on the magnitude of a single trade's P&L. That one row alone produced the account's reported `total_return_pct = 814.86%`; every other closed trade nets to a small honest loss. Both bad trades predate (by ~40 minutes) the `1b4ae7d` live-quote deviation guard (`_sane_vs_eod`, `backend/paper_trading/paper_trade.py:172-183`) — not a regression of that fix, leftover damage from the same bad-tick class that landed just before the guard went live. DB EOD prices for HAL were clean throughout (confirmed via `daily_prices`); the corruption is 100% from the live-quote path.

**Data correction** (`aqrti.db` backed up first as `aqrti.db.bak-20260714-212509-pre-paper-trade-fix`): trade #33's exit corrected from the fake ₹35.17 to the real EOD close (₹4442.40, net of NSE sell cost) — honest pnl -₹80.47, not -₹6928. Trade #35 deleted outright (should never have opened). Trade #36 / its open `PaperPosition` — a real, correctly-priced HAL re-entry, but sized via `pv * 0.07` off the *inflated* portfolio value from #35's fake gain (₹64,125 capital instead of a normal ~7%-weight amount) — rescaled to the honest capital (₹10,451.63) it would have received had #35 never existed. Portfolio row recomputed directly from cash + sum of open positions rather than hand-patched: `total_return_pct` now **-0.28%**, `total_value` ₹99,719.30 vs ₹100,000 initial — a small, plausible loss consistent with the account's actual trading history.

**Code fixes in `backend/paper_trading/continuous_monitor.py`** (two layers, both needed — the deviation guard alone didn't stop this because it only gates *fill-price selection*, not the downstream trigger-and-book step):
1. **Magnitude circuit breaker** in `_check_exits()`: any exit whose price vs. entry-price ratio exceeds `MAX_PLAUSIBLE_PRICE_RATIO = 4.0` is now skipped outright (logged as a rejected bad tick) instead of being trusted as a real stop-loss/take-profit trigger, independent of whether the quote passed `_sane_vs_eod` upstream.
2. **Same-cycle re-entry hole closed**: `_check_exits()` now returns the set of symbols it closed this heartbeat; `_check_entries()` takes a new `skip_symbols` param and excludes them, so a symbol just closed on a suspicious tick can no longer be immediately re-bought at the same bad price within the same 5-minute cycle. `run_continuous_monitor()` wires the two together.

**Verified:** backend stopped for the DB write (was PID 20508, live traffic present — confirmed cleanly down via `netstat` before touching `aqrti.db`), restarted after, `/health` green. `GET /api/v1/paper-portfolio` confirms the corrected live numbers (`totalReturnPct: -0.2807`, `currentCash: 53753.86`, `totalValue: 99719.3`) with all positions (HAL, DRREDDY, INFY, HDFCBANK, ICICIBANK, LT, NTPC) showing sane entry/current prices and weights. `python -m py_compile` clean on the modified file.

## [2026-07-14l] — Fix fitness_score: was 100% in-sample, hid OOS failure behind high scores (91+ fitness on strategies with negative OOS Sharpe)

**User report:** "there r stratergies with 91 fitness, is that even real and if so why isnt algo arena training them on real data?"

**Investigation:** `fitness_score` is real and honestly computed — but `backend/strategies/fitness_engine.py` never referenced `oos_sharpe`/`oos_win_rate`/`oos_passed` anywhere (confirmed by grep, zero hits). It scores 6 dimensions (profitability/consistency/robustness/cost/regime/longevity) purely from in-sample backtest stats. `breadth_momentum` — 421 of 876 candidates, the dominant family by volume — averaged in-sample fitness 69 (top scorers 91+) while averaging **oos_sharpe = -1.19** and passing OOS on only 14/415 (3.4%). Promotion itself was NOT broken: `strategy_lifecycle.py::promote_strategy()` correctly hard-gates on `oos_passed` (line 233) before any candidate reaches the arena, so every one of these 91-fitness rows was already being rejected. The arena's `eligible_strategies: 1` in `/health` is the honest consequence of a real ~2% OOS pass rate across the current template families, not a training/wiring bug. Strategy generation itself is confirmed healthy: 123 new candidates today, 279 in the last 3 days, most recent timestamped minutes before this check.

**The actual problem:** the score itself was misleading before promotion ever ran — a human (or a dashboard) reading "91.42 fitness" reasonably reads that as "excellent," when the same row's OOS Sharpe is -1.24 and it's a guaranteed reject. This is the same class of trap CLAUDE.md already warns about for win-rate-only evaluation, just on the fitness_score field instead.

**Fix (user-chosen: OOS penalty multiplier, not a full rebuild):** `backend/strategies/fitness_engine.py` — new `oos_penalty_multiplier(oos_passed, oos_sharpe)`: returns `1.0` when `oos_passed is None` (OOS not yet computed — fresh candidates still get scored purely in-sample for early evolution/selection, unchanged behavior), else `0.3` base multiplier for `oos_passed=False` further scaled down by how negative `oos_sharpe` is (floor -2.0, ceiling +1.2), or `1.0` for `oos_passed=True` (passing strategies are untouched). `compute_fitness()` now accepts `oos_passed`/`oos_sharpe` params and returns `fitness_score` as the OOS-adjusted composite, plus `fitness_score_in_sample` (old behavior, unpenalized) and `oos_penalty_multiplier` for debugging. `score_strategy()` passes `strategy.oos_passed`/`strategy.oos_sharpe` from the row.

**No extra wiring needed for propagation:** `strategy_research_loop.py` already calls `backtest_and_update()` (writes OOS fields) at line 122 then `score_all_strategies()` (recomputes fitness) at line 292 in the same daily loop — so the fix takes effect automatically on the next scheduled run. Also ran `score_all_strategies()` once manually against the live DB to apply it immediately rather than waiting.

**Verified against the exact reported row** (`AQRTI_STR_F6FADB0528`, sharpe=1.41, win_rate=49.7%, oos_sharpe=-1.24, oos_passed=False): reproduced the stale `fitness_score=91.42` with `oos_passed=None` (confirming the fitness math itself is unchanged), then confirmed `91.42 → 16.96` once real OOS data is factored in. Ran the full re-score against the live DB (526 strategies, no lock errors, backend stayed up throughout): `breadth_momentum` average fitness dropped 69 → 15.2, **zero strategies in the entire population now score ≥85** (was several). The one already-`promoted` strategy (`AQRTI_STR_74EEEE1B41`, oos_passed=True, oos_sharpe=0.954) is barely affected (scores 65.0, still comfortably clears every gate) — passing strategies keep their earned score. Full test suite: 87/87 passing, no regressions.

## [2026-07-14k] — Fix dormant invalid NVIDIA_NIM_MODEL fallback (z-ai/glm-5.2 → meta/llama-3.1-8b-instruct)

**Scope:** during gstack browser verification of this session's 4 new features, re-checked a limitation the insider-trading-scraper agent flagged but didn't fix: `backend/aqrti/llm/nvidia_nim.py:32` and `backend/intelligence/research_synthesizer.py:416` both defaulted `NVIDIA_NIM_MODEL` to the invalid string `z-ai/glm-5.2` when the env var is unset — CLAUDE.md explicitly calls this string invalid and names `meta/llama-3.1-8b-instruct` as the correct default. Currently harmless (`.env` sets `NVIDIA_NIM_MODEL` explicitly) but a real bug for any environment missing that var — it would silently call an invalid model.

**Fix:** both fallback defaults (and the docstring in `nvidia_nim.py`) updated to `meta/llama-3.1-8b-instruct`. Grepped the full `backend/` tree afterward to confirm no other occurrences of the invalid string remain.

**Verified:** full suite `87 passed` (no regression). Backend was kept running per user request — not restarted, since the fix only changes a fallback default that `.env` already overrides; nothing live depended on this code path in the current process.

## [2026-07-14j] — Fix test fixture gap from concurrent overfitting-gate change (2 failing tests)

**Scope:** four features (Monte Carlo/DSR overfitting gate, trade reconciliation, insider-trading scraper, Pydantic LLM schemas) were built in parallel by separate agents this session. The Monte Carlo/DSR gate (`[2026-07-14h]`) added a new promotion check requiring `mc_bankruptcy_pct`/`deflated_sharpe` to be non-`None`, but the shared test fixture `_make_promotable_strategy_row()` in `backend/tests/test_core.py` (used by `TestMinOosWinRateGate` and `TestExpectancyGate` to build synthetic rows that clear every gate except the one under test) predates those fields and left them `None`, causing 2 pre-existing tests to fail closed against the new gate.

**Fix:** added `mc_bankruptcy_pct = 0.0, deflated_sharpe = 1.0` to the shared fixture's base dict (`backend/tests/test_core.py`, `_make_promotable_strategy_row()`) — passing values, consistent with the fixture's own stated purpose of clearing every gate except the one each test targets. Did not touch the gate logic itself (`strategy_lifecycle.py`).

**Verified:** full suite `87 passed` (was 85 passed / 2 failed). Backend was kept running throughout per user request; confirmed still healthy via `/health` after the fix (no restart needed — test-only change).

## [2026-07-14i] — Trade Reconciliation: algo-suggested vs human-executed (slippage + hesitation tracking)

**Scope:** Personal Portfolio module extension per CLAUDE.md's real-money rules — tracks the gap between what an algo suggested (`PaperPosition`, simulated) and what the user actually did with real money (`PortfolioTransaction`, append-only tax audit trail), without ever merging the two source tables. Two things tracked: (1) market slippage — algo's suggested entry price vs the user's real recorded fill price; (2) human override drag — suggestions the user skipped, or entered late (days-to-fill = hesitation).

**Discovered a related-but-different existing module before writing anything:** `backend/portfolio/paper_real_reconciliation.py` (built 2026-07-11c, `GET /api/v1/reconciliation`) already compares `PortfolioTransaction` against **closed `PaperTrade`** rows (paper-trading engine's completed simulated trades) — but it has no persisted bridge table and no concept of a *skipped* suggestion (a `PaperPosition` the user never acted on). This task's ask is specifically about `PaperPosition` (live suggestions, including ones never taken), so it's additive, not a duplicate — kept both, didn't touch the existing one beyond adding a sibling route.

**New model `TradeReconciliation`** (`backend/aqrti/database/models.py`, ~line 2511, table `trade_reconciliations`): bridge/reporting table only — `algo_suggestion_id` (FK → `paper_positions.id`), `human_transaction_id` (FK → `portfolio_transactions.id`, nullable), `algo_suggested_price/date`, `human_fill_price/date`, `slippage_pct`, `days_to_fill`, `outcome_if_taken` (% move from algo's suggested price to latest close — the "what if" for skipped/pending suggestions), `status` (`suggested_only`/`matched`/`skipped`). Migration `backend/scripts/migrations/0007_add_trade_reconciliation.py`, applied and verified against the live DB.

**Matching logic** (`backend/portfolio/trade_reconciliation.py`, new): `ensure_suggestion_rows()` creates one row per `PaperPosition` (idempotent), `match_transaction()` links a real `buy` transaction to the closest unmatched suggestion within ±14 days on the same symbol (no fabricated matches — returns `None` if nothing plausible exists), `mark_stale_as_skipped()` ages 21-day-old unmatched suggestions into `skipped` (the raw signal for "override drag"), `backfill_from_existing_transactions()` does a one-time best-effort pass over any pre-existing `PortfolioTransaction` rows. Never joins `PaperPosition`/`PortfolioTransaction` directly in a query — asserted by a dedicated test (`test_report_never_mixes_paper_and_real_tables_directly`).

**Route:** `GET /api/v1/reconciliation/trades` added to the existing `reconciliation.py` router (chose this mount over inventing `/api/v1/portfolio/reconciliation` — `/api/v1/portfolio` is the paper-trading engine's router, and there is no real-money Personal Portfolio route file or UI page yet despite `PortfolioTransaction`'s schema existing; mounting under the established `/reconciliation` prefix avoided implying this bridges into the paper-engine population).

**UI:** new "Trade Reconciliation" panel on the Risk Center page (`ui/index.html`, after the stress-test panel), hydrated by `hydrateTradeReconciliation()` in `ui/pages/risk.js` (called from `hydrateRisk()`), backed by `Api.tradeReconciliation()` in `ui/api.js`. Shows avg slippage, avg days-to-fill, skipped count + avg missed-move, and a per-suggestion table. Resolves to real data / "Backend offline" / an honest "No reconciliation data yet" empty state — never a permanent spinner. Carries the "Suggestions, not advice — you decide" framing in the panel header per CLAUDE.md.

**Verified against the live DB and running backend, not just unit tests:** `PortfolioTransaction` has 0 rows (user hasn't recorded a real transaction yet) and `PaperPosition` has 7 live suggestion rows. Restarted the backend to pick up the new route (it was already running from before this session's edits) and hit `GET /api/v1/reconciliation/trades` directly — returned HTTP 200 with **7 real `suggested_only` rows** sourced from the actual `PaperPosition` table (HAL, INFY, DRREDDY, HDFCBANK, ICICIBANK, LT, NTPC) and an honest summary: `"note": "No reconciliation data yet — no real transactions recorded in PortfolioTransaction to match against algo suggestions."` This is the correct, expected state — no backfill matches exist because there is nothing real to match against yet; the table stops being empty the moment the user records their first real transaction near a suggestion. The existing `GET /api/v1/reconciliation` route was re-verified unaffected (still 200).

**Tests:** 7 new (`TestTradeReconciliation` in `backend/tests/test_core.py`) covering honest-empty-report, idempotent suggestion-row creation, slippage/days-to-fill computation, out-of-window non-matching, stale→skipped aging, no-fabrication backfill, and the paper/real no-direct-join guarantee. All 11 reconciliation tests (4 existing + 7 new) pass. Two unrelated pre-existing failures (`TestMinOosWinRateGate`, `TestExpectancyGate`) were investigated and are **not caused by this work** — they come from another concurrent session's in-progress overfitting-gate change (`[2026-07-14h]`, uncommitted at the time of this session) requiring `mc_bankruptcy_pct`/`deflated_sharpe` diagnostics those older test fixtures don't set up; confirmed by isolating this session's reconciliation-only files and running them independently (all pass). Not fixed here — not this session's code to touch.

## [2026-07-14h] — Monte Carlo permutation test + Deflated Sharpe Ratio (Lopez de Prado) added as new promotion gates

**Why:** CLAUDE.md's "prosecute your own results" standard — a strategy can clear every existing gate (fitness, WR, Sharpe, OOS, benchmark, duplicate) and still be a lucky draw from the strategy-generation process. These two published statistics attack that directly and independently of the existing gates.

**`backend/strategies/strategy_metrics.py` (new functions, ~line 101 onward):**
- `monte_carlo_permutation_test(trade_returns, n_simulations=1000)` — shuffles the TRADE-LEVEL P&L ordering (not price data) n_simulations times, rebuilds the compounding equity curve per shuffle, and reports `bankruptcy_pct` (% of shuffles whose cumulative equity hits <= 0) and `worse_sharpe_pct` (% of shuffles with worse per-trade Sharpe than the actual unshuffled order). Returns `{"bankruptcy_pct": 0.0, "worse_sharpe_pct": 0.0, "n_simulations": 0}` honestly for <5 trades rather than fabricating a number from noise.
- `compute_deflated_sharpe_ratio(sharpe, n_trials, n_returns, skew=0.0, kurtosis=3.0)` — standard Bailey & Lopez de Prado (2014) formula: expected-max-Sharpe-under-null via the Euler-Mascheroni-constant extreme-value approximation, Sharpe standard error corrected for skew/kurtosis (Mertens 2002), returns the DSR **statistic** itself — P(true Sharpe > 0), bounded [0,1] — not a rescaled Sharpe number. Uses `scipy.stats` (already a pinned dependency, `requirements.txt:18`, `scipy==1.13.0` — no new dependency added).

**`backend/strategies/strategy_backtester.py`:** `BacktestResult` dataclass gained `mc_bankruptcy_pct`, `mc_worse_sharpe_pct`, `deflated_sharpe` fields (~line 316-322). `compute_metrics()` now takes an `n_trials: int = 1` param and computes both new stats from the trade-level (not daily) return sequence after the existing Sharpe/Sortino/drawdown block. `backtest_strategy()` also gained `n_trials: int = 1`, threaded through to `compute_metrics()`. The main promotion-path upsert dict (~line 1367-1369) now writes all three fields onto `strategies_v2`.

**`backend/scripts/walk_forward_templates.py`:** its `backtest_strategy()` call now passes `n_trials=len(TEMPLATE_FAMILIES)` (currently 11) — the honest trial count for the Deflated Sharpe's multiple-testing correction, since this script IS the multi-template comparison the DSR formula is designed to correct for. The main candidate-generation/promotion path still defaults `n_trials=1` (single strategy evaluated, no population-wide correction available at that call site) per the task's explicit scoping — **flagging as a known limitation**: promotion-time DSR is therefore NOT corrected for the fact that ~870 candidate strategies are generated and screened, only for the 1-trial null. Wiring `n_trials` into the main generation/promotion pipeline (using the true candidate-population size) is a natural follow-up, not done here.

**`backend/strategies/promotion_config.py`:** new constants `MC_MAX_BANKRUPTCY_PCT = 15.0` (reject if >15% of shuffled trade orderings go bust) and `MIN_DEFLATED_SHARPE_PROB = 0.95` (reject if P(true Sharpe > 0) < 0.95). Existing constants untouched.

**`backend/strategies/strategy_lifecycle.py::promote_strategy()`:** new gate inserted after the existing OOS win-rate check, before the benchmark gate. Fails closed on `None` (pre-migration rows, "not yet computed" is not a pass) with the same `{"success": False, "error": ...}` shape as every other gate. Verified the gate sits correctly in the stack: dry-ran `promote_strategy` against the two highest-fitness `candidate` strategies (`AQRTI_STR_F6FADB0528` fitness 91.42, `AQRTI_STR_16196470BF` fitness 91.35) and confirmed they still reject at the pre-existing win-rate gate (49.7%/49.4% < 52%) exactly as before — the new gate never fires ahead of earlier gates, no regression to gate ordering.

**Migration `backend/scripts/migrations/0008_add_overfitting_gate_columns.py`** (renumbered from an initial `0007_*` after discovering a same-numbered `0007_add_trade_reconciliation.py` had landed concurrently from another session — renamed the file, `MIGRATION_ID`, and the already-applied `schema_migrations` row to `0008_*` to keep the sequence contiguous per the migrations README; both migrations' actual DDL was unaffected). Adds `mc_bankruptcy_pct`, `mc_worse_sharpe_pct`, `deflated_sharpe` (all nullable FLOAT) to `strategies_v2`. Applied and verified live via `PRAGMA table_info(strategies_v2)`.

**Spot-check (verified reality, not aspirational):** recomputed both new stats standalone against the one currently-promoted strategy's real persisted trades (`AQRTI_STR_74EEEE1B41`, `rl_momentum` family, 216 real trades from `strategy_backtest_trades`): `monte_carlo_permutation_test` → `bankruptcy_pct=0.0, worse_sharpe_pct=0.0` (sane, in [0,100], plausible for a positive-expectancy series with no single catastrophic loss); trade-level Sharpe 2.08; `compute_deflated_sharpe_ratio` → 1.0 at both n_trials=1 and n_trials=11 (in [0,1], correctly saturating given how far this Sharpe clears the expected-max-under-null bar). Stress-tested edge cases: a synthetic single -100%-trade sequence correctly drove `bankruptcy_pct` to 100.0 regardless of shuffle order; a synthetic negative-Sharpe (-0.77) and a near-zero-mean noisy series both correctly drove `deflated_sharpe` to 0.0. That one promoted strategy's own `mc_bankruptcy_pct`/`deflated_sharpe` DB columns are still `NULL` (it was backtested before this change and hasn't been re-backtested since) — will populate honestly the next time it goes through `backtest_strategy()`.

**Limitations / TODO, stated honestly:** (1) main promotion path's `n_trials` defaults to 1 — DSR at promotion time is not yet corrected for the true ~870-candidate population size, see above. (2) The one existing promoted strategy has not been re-backtested under this change yet, so its new gate fields are NULL in the DB right now (harmless — the gate only blocks NEW promotions, and `row.status not in ("candidate","shadow")` already short-circuits re-evaluation of already-promoted strategies). (3) No candidate strategy in the current population reaches the new gate in practice yet, since all currently fail earlier gates (OOS or win-rate) first — the gate is wired and unit-verified via direct function calls, not yet observed blocking a strategy that would otherwise have been promoted.

## [2026-07-14g] — NSE insider trading (PIT/SEBI reg 7(2)) scraper — real endpoint verified, 231 real disclosure rows landed, wired into research synthesizer

**Investigation first, per the hard-rule gate on fabricated sources.** Manually verified NSE's `https://www.nseindia.com/api/corporates-pit` endpoint against live traffic before writing any code — same API family, session/cookie/header pattern as the already-working `corporate_scraper.py` (homepage visit mints an Akamai `AKA_A2` cookie even when the homepage itself returns 403; the PIT API call still succeeds with that cookie — same tolerance the existing corporate scraper already relies on). Unlike `corporates-corporateActions`, `corporates-pit` requires a per-symbol `symbol=` param — an unscoped call returns only a market-wide `acqNameList` with no per-symbol rows. Confirmed the response is real, dated, named-individual regulatory data (e.g. a real ESOP acquisition by a named DRREDDY executive, Infosys Employee Benefits Trust transactions with real INR values and real XBRL attachment URLs), not a stub.

**New model `NSEInsiderTrading` (`backend/aqrti/database/models.py`, table `nse_insider_trading`):** fields mapped directly from NSE's actual field names (`acqName`→`person_name`, `personCategory`, `acqMode`→`mode_of_acquisition`, `secAcq`→`quantity`, `secVal`→`value_inr`, `befAcqSharesNo/Per`, `afterAcqSharesNo/Per`, `anex`→`regulation` e.g. "7(2)", `xbrl`→`attachment_url`), not guessed. `transaction_type` (buy/sell/pledge/other) is derived from NSE's `tdpTransactionType` + buy/sell value/quantity fields since NSE doesn't always populate a clean type field. Unique constraint `(symbol, disclosure_date, person_name, source_id)`.

**New scraper (`backend/data_supremacy/insider_trading_scraper.py`):** `scrape_insider_trading(db, symbols=None, from_date=None, to_date=None)` — defaults to the active curated universe minus VOO/QQQ (no NSE PIT feed for US tickers, same `NON_NSE_SYMBOLS` convention as research_synthesizer), one API call per symbol (NSE's constraint, not a design choice), default 90-day lookback. `backfill_insider_trading(db, days=365)` — no chunking needed, NSE tolerated a full 365-day range in one call per symbol in testing. Registered as pipeline step 8H in `data_supremacy/pipeline.py` (same try/except/results-dict pattern as every other step).

**Wired into `intelligence/research_synthesizer.py` as the 5th citation source**, following the exact existing pattern: import added, `INSIDER_LOOKBACK_DAYS = 90` (matches `FILING_LOOKBACK_DAYS`), `_collect_sources` queries `NSEInsiderTrading` point-in-time (`disclosure_date <= as_of_date`), `_format_source_line` renders a `[NSEInsiderTrading:id]` tag with person/category/transaction/quantity/value/holding-after, `_validate_citations`'s `date_col` map got `"NSEInsiderTrading": "disclosure_date"` — the anti-fabrication citation gate was NOT weakened, the new table just participates in it like every other source.

**Verified live, not just imported clean:** `Base.metadata.create_all` created the table; ran `scrape_insider_trading` against the real endpoint with a 365-day window across all 9 curated symbols — **231 real rows stored** (INFY 225, DRREDDY 6; the other 7 curated symbols — BEL, HDFCBANK, NTPC, ICICIBANK, CDSL, LT, HAL — had zero PIT disclosures filed in the trailing year, an honest data-availability finding, not a bug). Spot-checked rows directly in `aqrti.db`: real names, real dates, real INR values. `DataSourceHealth` row confirmed (`nse_insider_trading`, status=ok, records_fetched=231). Confirmed `_collect_sources`/`_format_source_line` wiring executes correctly end-to-end against the new table (tested against INFY, which has rows in-window; none of the curated 9 currently have a disclosure inside the 90-day synthesis lookback, so no live citation has fired yet — expected given how sparse PIT filings are, will surface naturally once a curated symbol gets a fresh one).

**Unrelated bug noticed in passing, not fixed (flagging per CLAUDE.md "never fixed silently"):** `research_synthesizer.py::_active_model_name()` defaults `NVIDIA_NIM_MODEL` to `"z-ai/glm-5.2"` if the env var is unset — CLAUDE.md explicitly documents this model string as invalid (`meta/llama-3.1-8b-instruct` is correct). Confirmed `.env` currently sets `NVIDIA_NIM_MODEL=meta/llama-3.1-8b-instruct` explicitly, so this is dormant, not live-broken — but the fallback default itself is still wrong and would bite silently if `.env` is ever regenerated without that line. Worth fixing in a future session.

## [2026-07-14f] — Pydantic structured-output pattern for new LLM call sites (`ask_structured`)

**Scope:** establish a fail-closed, schema-validated pattern for FUTURE LLM integrations, without touching the existing anti-fabrication citation gate in `intelligence/research_synthesizer.py` (`_validate_citations`/`_validate_synthesis_fields`/`_parse_llm_response` are unchanged — user decision, that code is proven and stays hand-rolled). No caller wired up yet; this is infrastructure for the next LLM feature (e.g. a filing-type classifier) to use instead of re-inventing dict parsing.

**`backend/aqrti/llm/json_utils.py` (new):** `strip_code_fences()` extracted verbatim from research_synthesizer's private `_strip_code_fences` so it's shared, not duplicated. `intelligence/research_synthesizer.py` now imports it (`from aqrti.llm.json_utils import strip_code_fences`) and the private copy was deleted; its citation/field validation logic is untouched.

**`backend/aqrti/llm/provider.py::ask_structured()` (new, ~line 71):** `ask_structured(prompt, response_model: Type[BaseModel], system=None, model=None, temperature=None, max_tokens=None) -> BaseModel | None`. Appends `response_model.model_json_schema()` to the prompt in the same "STRICT JSON ONLY" house style as research_synthesizer's prompt, strips code fences, and parses via `response_model.model_validate_json()`. **Fail-closed:** returns `None` on provider-call exception, `json.JSONDecodeError`, or `pydantic.ValidationError` — logs the raw response + error, never fabricates a partial/guessed object.

**Test (`backend/tests/test_ask_structured.py`, new):** two pytest cases — (1) a **live** call against the configured provider (`nvidia_nim`, skipped if unconfigured) using a trivial 2-field `CapitalAnswer(country, capital)` schema; (2) a monkeypatched-garbage case proving fail-closed returns `None` rather than a guessed object. **Verified live:** `ask_structured("What is the capital of France?...", CapitalAnswer)` against NVIDIA NIM returned `CapitalAnswer(country='France', capital='Paris')` — correctly typed, schema-valid. Full suite: 78 → 80 tests, all passing.

## [2026-07-14e] — News machine overhaul: symbol-targeted scrapers, NIM batch news analysis, live-only news gate for paper trading, NIM agent briefs, news UI upgrade

**Scope (user-approved plan):** better articles for the curated 9, all news through NVIDIA NIM for analysis (40 RPM respected), news informs LIVE trading decisions only — never backtests (explicit user constraint), agents audited + NIM-powered briefs, news UI improved. All 78 tests passing (5 new).

**Symbol-targeted Google News scraper (`news/news_collector.py`):** the 5 generic market RSS feeds gave BEL/NTPC/CDSL/DRREDDY/LT/HAL near-zero coverage (confirmed 2026-07-12 — the root cause of research-conditioned templates staying zero-trade). Added per-symbol Google News RSS search (free, keyless) built from `ENTITY_MAP`'s most specific alias, 15 articles/symbol cap, `symbol_hint` plumbed through parser→pipeline as fallback attribution (never overrides an extracted match; recorded as `mention_type="search_target"`). **Verified live: 135 articles, 15 per symbol for ALL 9 — first-ever coverage for the six starved names; pipeline run stored 185 new rows vs ~50/run before.**

**NIM batch news analyzer (`news/llm_analyzer.py`, migration 0006):** recent curated-symbol articles go to NVIDIA NIM in batches of 10 per request, ≤6 requests/run (≤15% of one minute's 40 RPM budget, shared limiter in `nvidia_nim.py` still applies). LLM judges sentiment/sentiment_score/event_type/relevance per article; anti-fabrication ID-echo gate (same discipline as research_synthesizer's citation gate) — a verdict whose id wasn't in the sent batch is discarded, unreturned articles stay keyword-scored. Provenance columns `llm_analyzed`/`llm_relevance` on `news_events` so LLM-refined rows are always distinguishable. Wired as scheduler Step 3B (after collection, before sentiment/synthesis) + boot step 3. **Verified live: 60 articles refined in exactly 6 requests; spot-check sane (generic "stocks to watch" mention → relevance 0.10; ICICI-specific analyst piece → 0.90).**

**Live news gate (`news/live_news_gate.py` + `paper_trading/paper_trade.py::open_position`):** fresh (<24h) LLM-verified negative news (score ≤ -0.5, relevance ≥ 0.6) on a symbol blocks NEW paper-trade entries on it. Deliberately narrow, fail-open on any query error, skip-not-close. **LIVE PATH ONLY by user decision — the backtester and arena replay engine have no such gate; historical news is too thin to simulate it honestly and retro-fitting would be look-ahead contamination.** Unit-tested (blocks on qualifying news; ignores keyword-only rows, low relevance, stale news).

**Agents audit + NIM briefs (`agents/agent_base.py`):** all 14 agents verified firing (every one reported at today's boot, 0 failed tasks in 3 days — "arena idle" was the only prior agent-adjacent issue and was fixed in 2026-07-14d). Added `_attach_llm_brief`: one NIM request per agent per cycle (≥2 findings only, ~14 req/hourly cycle) turns rule-based findings into a 2-3 sentence executive brief, explicitly suffixed "(AI brief)" per the honesty rule; rule-based summary kept in `summary_raw`; fail-open. **Verified live: news agent produced a real brief citing actual scraped stories.**

**News UI (`ui/pages/news.js`):** headlines are now clickable source links, LLM-analyzed rows carry an "AI n%" provenance badge (relevance in tooltip), and the feed gets per-symbol filter chips. API (`get_recent_news`) now returns `llm_analyzed`/`llm_relevance`. Cache-busted `?v=20260714a`.

**RPM budget accounting (worst case per hour):** synthesis 9 + news analysis 6 + agent briefs 14 ≈ 29 requests spread across an hour — the 40/min client-side limiter is never stressed.
...
- `backend/.env` — key activated: `AQRTI_FINNHUB_API_KEY=<redacted, see .env>`
