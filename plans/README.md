...
> Terminal runs in-browser via `npm run dev`. A desktop Electron build exists but is not current — use the browser UI.
...
| **Desktop** | Electron (stale build — not current; browser UI recommended) |
...
### 3 — Desktop App (stale — browser UI preferred)

An Electron build exists under `dist/installers/` but has not been rebuilt since June 2026 and does not reflect recent UI changes. Use the browser UI (`npm run dev`) instead. Rebuild with `npm run build:electron` when the UI stabilises.
