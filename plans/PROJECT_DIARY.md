...
Last synced with project state: **2026-07-06** (through CHANGELOG entry `2026-07-06d` — AQRTINet structural bug fixes: regime routing, CV leaks, confident-label weighting — see Timeline phase L).
...
### Model ensemble (current, as of 2026-07-06 — AQRTINet v4.1)
| Model | Role |
|---|---|
...
| **AQRTINet v4.1** (custom) | In-house model — asymmetric trading loss (false positives penalized 1.5× harder, softened from 2.0), regime-aware mixture of experts (separate gradient booster per BULL/BEAR/SIDEWAYS/VOLATILE regime), cross-sectional percentile ranking, 7-fold out-of-fold stacking via TimeSeriesSplit (uses CatBoost/NGBoost predictions as meta-features), 5-fold TimeSeriesSplit Platt-scaled calibration for well-behaved P(UP) outputs, per-regime balanced-accuracy-optimal decision threshold, per-row historical regime routing |

### AQRTINet v4.1 — structural bug fixes applied 2026-07-06
- **Per-row regime routing (v4.1 fix for the HIGH inference bug)**: inference `_predict_impl`/`_predict_proba_impl` now route each row to its own regime expert via `_row_regimes()`, which checks `self._predict_dates` (set by `BaseModel.predict(dates=...)`). Callers without dates (live single-day inference) fall back to today's regime — correct in that case. Historical/backtest evaluation in `walk_forward.py`, `backtest_validator.py`, `model_retrainer.py`, and `compare_models.py` all now pass per-row dates.
- **Training regime plumbing (v4.1 fix for the HIGH training bug)**: `walk_forward.py` and `backtest_validator.py` now inject `_training_dates` before `fit()`, matching `model_retrainer.py`'s existing correct behavior. `DataSplit` carries `train_dates`/`test_dates` populated from the source df.
- **Inner CV leakage fixed**: both the stacking OOF loop and Platt calibration fold loop use `TimeSeriesSplit` instead of `StratifiedKFold(shuffle=False)`, preventing future-leakage where early-chronology rows were predicted by models trained on later rows.
- **Confident-label weighting now actually works**: previously a silent no-op because `training_dataset.py` strips `return_5d` from feature columns. Callers now set `model._return_5d` before fit (from `DataSplit.return_5d_train` / `get_final_train_test()` output); the function reads from this attribute.
- **Softened asymmetric class weight**: `{0: 1.5, 1: 1.0}` (was 2.0) — the 2.0 weight suppressed recall to ~12.6% despite competitive AUC-ROC (0.645 vs CatBoost 0.634).
- **Per-regime decision thresholds**: `_find_f1_threshold()` sweeps 50 thresholds on Platt-calibrated OOF probabilities, picking the one maximising balanced accuracy (not F1 — F1 can pick degenerate all-positive thresholds on majority-class labels). Applied in `_predict_impl()`. Stored per-regime in the pkl.
...
| **L. AQRTINet Structural Bug Fixes** | 2026-07-06 | Fixed all 6 bugs from AQRTINET_IMPROVEMENT_PLAN.md: training regime collapse in 2/3 code paths, inference regime routing using today's regime, CV leakage in stacking and Platt calibration (StratifiedKFold→TimeSeriesSplit), confident-label weighting silent no-op, and predict_interval regime routing. Changed 6 files. Baseline unchanged: 927 algos, 0 promoted. |
| **K. Stage 1 Complete + GO-1/5b/5c + ARCH-3/4** | 2026-07-05 | Completed all remaining Stage 1 reliability items and advanced Stage 2/3/4. **ARCH-3:** `backend/aqrti/database/migrations.py` — `ensure_migrations_table()` + `run_pending()`; numbered idempotent migration scripts in `backend/scripts/migrations/`; `schema_migrations` table; `init_db()` calls both after `create_all`. Bootstrap migration `0001` included. **ARCH-4:** `backend/aqrti/scheduler.py` — standalone `python -m aqrti.scheduler` entry point; writes/removes `backend/scheduler.pid`; SIGINT/SIGTERM graceful shutdown; `time.sleep(60)` fallback on Windows. API `on_startup` detects external scheduler via `os.kill(pid, 0)` and skips embedded start. `GET /api/v1/system/scheduler-status` reports `mode: external|embedded|none`. `setup_watchdog_task.ps1` Task 3 added. **GO-5b:** Added 3 DSL families to `strategy_generator.py`: `relative_strength` (cross-sectional nifty_rs_21d + sector_rs_21d, 20-45d, weight=0.12), `breadth_momentum` (breadth_pct_above_ema50 filter, 15-35d, weight=0.09), `long_hold_momentum` (delivery_pct + ema50, 30-60d, weight=0.14 — highest). 5 new REGIME_FEATURES; MAX_HOLDING raised 60→65. **GO-5c:** `backend/scripts/go5c_oos_audit.py` — 4 checks, verdict PASS WITH WARNINGS (OOS/IS Sharpe overlap 3.4%, no selection pressure data yet, OOS windows reach ~2027-01-01 leaving no pristine holdout — deferred to first promoted algo). **GO-1:** `backend/aqrti/api/routes/go_nogo.py` — `GET /go-nogo` (5-condition scorecard + quarantine board + actionable signals) + `GET /go-nogo/uptime-log`; `hydrateGoNogo()` + `hydrateGoNogoUptime()` in `ui/app.js`; new "Go / No-Go" nav + page in `index.html`. All 5 conditions correctly red (0 promoted algos). Stage 1 uptime clock started 2026-07-05. |
| **J. Stage 1 Reliability Track** | 2026-07-05 | Completed Stage 1 reliability items: GO-2 (watchdog auto-restart: `backend/scripts/watchdog.py`, Windows Scheduled Task, amber UI topbar pill showing last restart); ARCH-2 (37-test suite in `backend/tests/test_core.py` — Sharpe/Sortino/MDD correctness, label-generator forward-label integrity, promotion gate sanity, session-rollback pattern; 37/37 passing); GO-5 (population diagnosis from live DB — median WR=47.1% below coin flip, median Sharpe=–2.1, only 4 algos pass Sharpe gate individually, all have < 60 trades; root cause is DSL entry conditions generating random-walk-frequency signals, not weak gates — full report at `docs/GO5_POPULATION_DIAGNOSIS.md`). Also completed from prior session: ARCH-1 (stray root DB archived), GO-12 (MIN_CONFIDENCE=60 confirmed), ARCH-7 (busy_timeout confirmed), ARCH-9 (weekly backup scheduler), GO-3 (pipeline health check + red UI banner), GO-4 (Telegram alerts). Current state: 0 promoted algos out of 1,134 population — still correct per honest gates. Stage 1 reliability items are now complete; next is GO-5b signal-quality work per the diagnosis verdict. |
| **I. Docs Audit + Placeholder-Data Purge + Obsidian Exporter** | 2026-07-05 | Full documentation review produced `IMPROVEMENTS.md`, a prioritized backlog (P0 placeholder-data violations, P1 stale/wrong docs, P2 historical-doc labeling, P3 polish). Working that backlog found and fixed three real fabricated-data violations beyond the ones already catalogued: `backend/seed_missing_data.py` (deleted — had written invented earnings actuals and `random.uniform()` options data into the DB as if real), a matching pattern in `fii_dii_scraper.py`'s history-backfill (deleted — wrote randomized FII/DII crore flows indistinguishable from real scraped rows), and a hardcoded fake "Today's Alerts" panel on the Overview page (`index.html`, never wired to JS at all — now hydrated from the real research-findings API). Removed `MOCK_AGENT_DATA`/`MOCK_VAULT_DATA`/dead `USE_MOCK` plumbing from the UI. README/USER_GUIDE corrected to match current code (model stack, family/mutation-op/fitness-dimension counts, algo naming). Also planned and built the Obsidian vault integration (`docs/OBSIDIAN_INTEGRATION_PLAN.md`): OBS-1 built `backend/obsidian/` (`vault_exporter.py`, `renderers.py`, `vault_writer.py`) rendering Daily/Stock/Lesson/Home notes from the DB into a one-way, idempotent, ownership-checked markdown vault — verified against the live DB (1044 notes on first run, 100% unchanged on re-run). OBS-2 then wired it in: Step 13 added to the daily scheduler pipeline (`aqrti/data/scheduler.py`, failure-isolated like every other step) and `POST /admin/obsidian-export?full=true` added to the API, both verified live. OBS-3 added Algo notes (promoted/active/retired `StrategyV2` rows rendered as plain-English DSL rules + shadow-trade record — 0 written, correctly matching the honest 0-promoted baseline) and per-agent Report notes (grouping `ResearchFinding` by agent+day — 69 generated). Still untouched: Portfolio notes (OBS-4, blocked on the My Portfolio module) and the phase-3 two-way vault-inbox idea (OBS-5, deferred). The vault then went **live**: `backend/.env` created with `AQRTI_OBSIDIAN_VAULT_PATH` pointing at `Desktop/AQRTI Vault/` (sibling of the repo, not inside it, per the spec), first real export wrote 1113 notes, idempotency re-confirmed against the real OneDrive-synced folder. The daily scheduler (Step 13) now keeps it current automatically. Finally, at the user's request the vault got real organization instead of a flat file dump: the Dataview community plugin was actually installed (not just documented as optional) into `.obsidian/plugins/dataview/` with DataviewJS disabled (declarative queries only); four `_index.md` notes (Daily/Stocks/Lessons/Algos, each a Dataview `TABLE` sorted by the metric that matters — fitness, severity, confidence, date) were added; `Home.md` became a real dashboard with quick-jump links, a live trend table, and honest explanatory copy instead of silence when a section is empty; and a CSS snippet (`.obsidian/snippets/aqrti-theme.css`, enabled by default) color-codes each note type in tag pills, the file explorer, and the graph view. Verified live: 5 new files written, re-run 1117/1117 unchanged. User feedback on the result ("make it show more useful info, i dont want to see random shit") surfaced three real bugs, not just presentation: Stock notes were reading `PaperTrade.actual_return` (NULL on nearly every row) instead of the populated `gross_pnl_pct`, so real P&L silently showed as "—" (AAPL: 10 closed trades, all +0.20%, all previously blank) — fixed, plus added a trade-record summary (win rate/avg return/total P&L). `exchange` was guessed "NSE" for every symbol via a meaningless suffix check, so AAPL showed `exchange: NSE` — fixed to look up the real `GLOBAL_UNIVERSE` metadata with a bare→`.NS`→`.BO` fallback (confirmed `INFY` bare vs `INFY.NS` are genuinely distinct real entries — the NYSE ADR vs the NSE listing — not a bug). Report notes dumped every `ResearchFinding` raw including agents re-logging identical running-status findings on every intra-day run (one day's `cro` report: 24 findings that were really 2 distinct messages repeated up to 12× as counters grew) — added dedup-by-title-with-digits-stripped, keeping only the final occurrence + an `(×N)` count. Net: Report notes shrank 10,824 → 2,773 lines (-74%), all signal. Re-verified idempotent after all fixes. |
