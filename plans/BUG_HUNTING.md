...
- **Verified:** 2026-07-07 — ran `run_sentiment_pipeline()` directly against the live DB; completed with `status: COMPLETED`, `regime: BULL MARKET`, no exception. (See C16 — that regime label was itself a separate bug, found and fixed in this session.)
- **Fixed:** 2026-07-07
...
## C16 — Arena replay engine (`arena/replay_engine.py`) applied ZERO NSE transaction costs
...
## C17 — Sentiment pipeline wrote non-canonical regime labels `"BULL MARKET"`/`"BEAR MARKET"` into `MarketRegime.regime`, silently breaking every regime-aware gate for the affected dates
...
## H33 — `promote_strategy`'s Telegram alert used a non-existent column and double-scaled win rate
