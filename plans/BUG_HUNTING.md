...
- **Verified:** 2026-07-07 — ran `run_sentiment_pipeline()` directly against the live DB; completed with `status: COMPLETED`, `regime: BULL MARKET`, no exception. (See C19 — that regime label was itself a separate bug, found and fixed in this session.)
- **Fixed:** 2026-07-07
...
## C18 — Arena replay engine (`arena/replay_engine.py`) applied ZERO NSE transaction costs

- **File:** `backend/arena/replay_engine.py` — `_simulate_day` (entry section ~line 491-505, exit section ~line 426-448 before fix)
- **Root cause:** Every other backtest/paper-trading path in the codebase enforces the 0.28% NSE delivery round-trip cost (`strategy_backtester.py`'s `_transaction_cost`/`ROUND_TRIP_COST`, `paper_trade.py`, and `continuous_monitor.py` after C1's fix). The arena's own replay engine — used by `arena_engine.run_arena_for_strategy` to grade champion candidates (120% return, <25% drawdown, >52% win-rate gates) — computed `gross_pnl`/`gross_pnl_pct` as pure `(exit_price - entry_price) * shares` with no cost deduction anywhere in the file, and credited `portfolio.current_cash` with that same gross PnL. `PaperTrade.gross_pnl`/`gross_pnl_pct` are explicitly named "gross" in the model but no corresponding net field or cost application existed in this module — the one backtest path in the system where the mandatory 0.28% round-trip was silently absent.
- **Impact:** The arena's champion gates (the harshest bar in the whole promotion pipeline, per the module's own docstring) graded every strategy on a return series with zero cost drag. A high-turnover strategy could clear "champion" status purely because transaction costs were never subtracted — the exact "0.28% round-trip must always apply" rule CLAUDE.md calls out as non-negotiable. Currently 0 arena champions exist in the live DB, so this had not yet produced a false champion in practice, but it was a live, severe miscalibration on the single harshest gate in the pipeline.
- **Fix:** Added `NSE_ROUND_TRIP_COST = 0.0028` (matching `strategy_backtester.ROUND_TRIP_COST`) split 55/45 buy/sell (matching `strategy_backtester._transaction_cost`'s split). Entry fills now load the buy-side cost into the stored cost-basis `entry_price` (`entry_fill = entry * (1 + NSE_BUY_COST_PCT)`), while `stop_loss_price`/`target_price` remain computed off the raw signal price so SL/TP still trigger against real market prices (matching `strategy_backtester.py`'s treatment). Exit fills net out the sell-side cost (`exit_fill = cur_price * (1 - NSE_SELL_COST_PCT)`) before computing `gross_pnl`/`gross_pnl_pct` and crediting `portfolio.current_cash`.
- **Verified:** 2026-07-07 — ran the same strategy (`AQRTI_STR_00177E0CC1`, 758 trades over 1yr replay) against an isolated scratch copy of the live DB with and without the fix. Before fix (no cost model): return=+27.16%, win_rate=51.8%, final_value=₹127,158. After fix (cost model applied): return=+0.42%, win_rate=47.0%, final_value=₹100,420. The ~26.7pp difference on 758 trades ≈ 0.28% × 758 / 2 (round-trip split across entry+exit) — matches the expected cost drag almost exactly, confirming both that the bug was real and that the fix applies the correct magnitude.
- **Fixed:** 2026-07-07

## C19 — Sentiment pipeline wrote non-canonical regime labels `"BULL MARKET"`/`"BEAR MARKET"` into `MarketRegime.regime`, silently breaking every regime-aware gate for the affected dates

- **File:** `backend/sentiment/market_sentiment.py:107,115` (`determine_regime`)
- **Root cause:** `determine_regime`'s own docstring states it returns one of `"BULL"|"BEAR"|"SIDEWAYS"|"VOLATILE"`, but the actual `return` statements emitted `"BULL MARKET"` and `"BEAR MARKET"` instead. This value flows unmodified through `sentiment_engine.run_sentiment_pipeline()` → `save_market_regime()` → the `market_regimes` table, which every regime-aware consumer in the strategy pipeline reads via exact string equality: `strategy_backtester._should_enter`'s `if regime not in allowed_regimes: return False` gate, `strategy_backtester._regime_on`'s day-by-day regime lookup, `fitness_engine.regime_adaptability_score`'s `mapping.get(current_regime, 0.0)` (returns 0.0 for any unrecognized key), `arena_engine`'s regime-stratified robustness grading, and `meta_learner._current_regime`. None of them recognize `"BULL MARKET"` — confirmed live: the `market_regimes` table had exactly 4 rows with `regime='BULL MARKET'` for 2026-07-01, 07-05, 07-06, and 07-07 (today), produced by the verification run logged in C9's fix ("regime: BULL MARKET" in that entry's own verification note) once C9 unblocked the sentiment pipeline from crashing before ever reaching the regime-write step.
- **Impact:** For the 4 most recent trading days (including today), every regime-gated consumer silently treated the current regime as an unrecognized value rather than BULL: entry gating in `_should_enter` would reject any strategy whose `allowed_regimes` list doesn't happen to contain the literal string `"BULL MARKET"` (none do, since they're all generated against the canonical 4-value vocabulary), and `fitness_engine`'s regime-adaptability dimension would silently score 0 regardless of the strategy's actual `bull_sharpe`. This is a silent-corruption bug with no exception anywhere in the chain — exactly the failure mode CLAUDE.md's bug-hunting brief calls out (a metric/gate quietly computing the wrong thing with no error signal).
- **Fix:** Changed both `return` statements in `determine_regime` to emit the canonical `"BULL"`/`"BEAR"` strings. Also corrected the 4 already-poisoned live rows in `market_regimes` (`UPDATE market_regimes SET regime='BULL' WHERE regime='BULL MARKET'`, and the equivalent for `'BEAR MARKET'`, which currently had 0 matching rows) after taking a DB backup (`backend/aqrti.db.bak-20260707`) per CLAUDE.md's bulk-mutation convention. Note: `aqrti/api/routes/overview.py:54` and `aqrti/api/routes/market_regime.py:92` also contain the literal strings `"BULL MARKET"`/`"BEAR MARKET"` — these are unrelated, self-contained UI-display-only heuristics computed directly from a NIFTY return threshold (not read from the `market_regimes` table and not consumed by any strategy-engine gate), so they were intentionally left untouched.
- **Verified:** 2026-07-07 — confirmed via direct SQL query that `market_regimes` had 4 rows with `regime='BULL MARKET'` before the fix (dates 2026-07-01, 07-05, 07-06, 07-07) and 0 after the `UPDATE`; re-ran `determine_regime()` with bull-condition and bear-condition inputs directly and confirmed it now returns `"BULL"`/`"BEAR"` respectively.
- **Fixed:** 2026-07-07
...
## H34 — `promote_strategy`'s Telegram alert used a non-existent column and double-scaled win rate

- **File:** `backend/strategies/strategy_lifecycle.py:237` (`promote_strategy`)
- **Root cause:** `alert_algo_promoted(strategy_id, row.sharpe_ratio or 0.0, (row.win_rate or 0.0) * 100)` — two independent bugs. (1) `StrategyV2` has no `sharpe_ratio` column (that field only exists on the unrelated `PerformanceSnapshot` model); accessing it raises `AttributeError`, which the surrounding bare `try/except Exception: pass` silently swallowed, so `alert_algo_promoted` never actually ran — no promotion alert was ever sent, for any promotion, ever. (2) Even if the attribute existed, `row.win_rate` is already stored as a percent (e.g. `64.0`, gated against `MIN_WIN_RATE=52.0` in `promotion_config.py`) — multiplying by 100 again would format as a nonsense value like "6400.0%" in the alert text, since `alert_algo_promoted`'s own formatting (`f"{wr:.1f}%"`) expects a plain percent value.
- **Impact:** Zero promotion alerts have ever been sent to Telegram (silent failure, no error surfaced anywhere — the promotion itself still succeeded, only the notification was lost). Had the AttributeError not existed, every alert that did fire would have shown a 100x-inflated, nonsensical win rate.
- **Fix:** Changed to `alert_algo_promoted(strategy_id, row.sharpe or 0.0, row.win_rate or 0.0)` — using the real `sharpe` column and passing `win_rate` through without re-scaling.
...
| 2026-07-07 | Batch 7: C13 (`run_full_training()` trained 0 models under recent-data-only policy — fixed), C14 (model filename collision between direction_5d/outperform_binary — fixed), C15 (`model_versions` unique constraint + registration lookup missing label_col — fixed + migration). H32 (3 quarantine-evidence endpoints counted non-shadow trades — fixed). New C16 found+fixed: `meta_learner`'s cross-family `bad_conditions` leak zeroed out `breadth_momentum`/`long_hold_momentum` strategy generation (both families were structurally family-blind to graveyard-condition blacklisting). New H33 found+fixed: evolution offspring (mutation/crossover) never passed the structural prescreen fresh candidates are held to. |
| 2026-07-07 | Batch 8 (parallel worktree session): arena/strategy-pipeline focused audit of `arena_engine.py`, `promotion_config.py`, `strategy_lifecycle.py`, `strategy_backtester.py`, `fitness_engine.py`, `meta_learner.py`, `arena/replay_engine.py`, `arena/strategy_merger.py`. New C18 found+fixed: arena replay engine (`arena/replay_engine.py`) applied zero NSE transaction costs — the one backtest path in the system missing the mandatory 0.28% round-trip; verified before/after on a live strategy (758 trades: +27.16% gross → +0.42% net). New C19 found+fixed: sentiment pipeline's `determine_regime()` wrote non-canonical `"BULL MARKET"`/`"BEAR MARKET"` labels instead of `"BULL"`/`"BEAR"` into `market_regimes`, silently breaking every regime-gated consumer (backtester entry gate, fitness engine's regime-adaptability score, arena robustness grading) for 4 recent live dates including today; fixed the labels and corrected the 4 poisoned DB rows (backup taken first). New H34 found+fixed: `promote_strategy`'s Telegram alert referenced a non-existent `StrategyV2.sharpe_ratio` column (silently swallowed by a bare except, so no promotion alert has ever been sent) and double-scaled `win_rate` by 100; fixed to use the real `sharpe` column and pass `win_rate` through unscaled. |
